const MAKE_WEBHOOK = process.env.MAKE_WEBHOOK || "https://hook.us2.make.com/5hdgapnpzhimd9ktarrovfkkc695krxx";
...
IF OWNER: Collect in order: property address, number of bedrooms and bathrooms, motivation for selling, timeline, condition (great/good/needs work/major repairs), asking price, mortgage balance. Then ask: "Would you prefer a quick 15-minute call with Diego, or would you like me to put together a ballpark offer right here over text?"
...
[LEAD name=X address=X beds=X baths=X motivation=X timeline=X condition=X asking=X owed=X calltime=X preference=X email=X]`;

function parseLead(raw) {
  const get = (key) => {
    const match = raw.match(new RegExp(key + "=([^=\\[\\]]+?)(?=\\s+\\w+=|$)"));
...
module.exports = async function handler(req, res) {
  ...
  if (leadMatch) {
    const lead = parseLead(leadMatch[1]);
    await fireWebhook(lead);
  }
